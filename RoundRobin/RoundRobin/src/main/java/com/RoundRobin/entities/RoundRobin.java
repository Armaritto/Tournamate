...
public RoundRobin(List<RoundRobinTeam> RoundRobinTeams)
{
    ...
    if(teams.size()%2==0)
    {
        ...
        {
            rounds.add(new RoundRobinRound( teams , teams.size()/2,i+1));
        }
    } else{
        numberOfRounds=teams.size();
        for (int i=0; i<numberOfRounds; i++)
        {
            rounds.add( new RoundRobinRound(teams,(teams.size()-1)/2,i+1));
        }
        }
}
    ...
    rounds.set(currentRound,new RoundRobinRound(roundRobinTeams,currentRound,(int)(teams.size()/2)));
